    ...
    <Column className="page-stack" maxWidth="l" fillWidth gap="128">
      ...
      <Column as="section" gap="40" aria-labelledby="featured-heading">
        <SectionHeading
          ...
          title={locale === "ko" ? "장고야 부탁해 — 혼자 설계하고 운영하는 모바일 제품" : "Jango — a mobile product designed and operated end to end"}
          description={locale === "ko" ? "냉장고 재고 관리에서 시작해 AI 추천, 공유 공간과 운영 도구까지 확장하고 있습니다." : "It began with refrigerator inventory and now spans AI recommendations, shared spaces, and operations tooling."}
        />
      ...
      <Column as="section" gap="40">
        ...
        <Grid columns="3" m={{ columns: 2 }} s={{ columns: 1 }} gap="24" fillWidth>
          {secondaryProjects.map((project, index) => (
            <ProjectCard key={project.slug} project={project} locale={locale} priority={index === 0} headingLevel="h3" />
          ))}
        </Grid>
      </Column>

      <Column as="section" gap="40">
        <SectionHeading eyebrow={labels.experience} title={locale === "ko" ? "공공 시스템에서 개인 제품까지" : "From public systems to an independent product"} />
        <Column gap="32">
            ...
            <Row key={experience.company.en} className="timeline-item" gap="24" fillWidth>
              ...
              <Column fillWidth gap="12" paddingBottom="24">
                <Row fillWidth horizontal="between" gap="16" s={{ direction: "column" }}>
                  <Column gap="4">
                    <Heading as="h3" variant="heading-strong-l">{localize(experience.company, locale)}</Heading>
                    <Text variant="body-default-m" onBackground="brand-weak">{localize(experience.role, locale)}</Text>
                  </Column>
                  <Text variant="label-default-s" onBackground="neutral-weak">{localize(experience.period, locale)}</Text>
                </Row>
                <Row gap="8" wrap>
                  {experience.projects.map((project) => <Tag key={project.en} size="s">{localize(project, locale)}</Tag>)}
                </Row>
              </Column>
            </Row>
          ))}
      ...
      <Column
        ...
        align="center"
      >
        <Text className="eyebrow" variant="label-strong-s" onBackground="brand-weak">{labels.contact}</Text>
        <Heading as="h2" variant="display-strong-s" wrap="balance">
          {locale === "ko" ? "제품과 팀에 대한 이야기를 나누고 싶습니다" : "Let’s talk about the product and the team"}
        </Heading>
        <Text variant="body-default-l" onBackground="neutral-weak" wrap="balance">
          {locale === "ko" ? "프로젝트나 채용에 관해 공유할 내용이 있다면 이메일로 연락해 주세요." : "If you have a project or role worth discussing, send me an email."}
        </Text>
        <Row gap="12" wrap horizontal="center">
          <Button href={`mailto:${person.email}`} variant="primary" prefixIcon="email">{person.email}</Button>
          <Button href={person.github} variant="secondary" prefixIcon="github">GitHub</Button>
        </Row>
      </Column>
    </Column>
  );
